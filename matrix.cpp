...
matrix::matrix(uint rows, uint cols)
    : m_{rows},
      n_{cols},
      buf_{(m_ == 0 || n_ == 0) ? nullptr : new double[m_ * n_]} {
...
matrix::matrix(matrix&& mat) : m_{0}, n_{0}, buf_{nullptr} {
    std::swap(m_, mat.m_);
    std::swap(n_, mat.n_);
    std::swap(buf_, mat.buf_);
}

matrix::matrix(const std::string& matstr) : m_{0}, n_{0}, buf_{nullptr} {
    std::string::const_iterator start = matstr.cbegin();
    std::string::const_iterator end;
    *this = parse_matrix(start, end);
}

matrix::~matrix() { delete[] buf_; }

uint matrix::m() const { return m_; }

uint matrix::n() const { return n_; }
...
bool matrix::is_empty() const { return m_ == 0 || n_ == 0; }
...
bool matrix::is_invertible() const { return det() != 0; }

matrix matrix::get_row(uint i) const {
    matrix a(1, n_);
    for (uint j = 0; j < n_; j++) a(0, j) = (*this)(i, j);
    return a;
}

matrix matrix::get_col(uint j) const {
    matrix a(m_, 1);
    for (uint i = 0; i < m_; i++) a(i, 0) = (*this)(i, j);
    return a;
}
...
void matrix::set_row(uint i, const matrix& row) {
    if (row.m_ != 1 || row.n_ != n_)
        throw bad_size();
    for (int j = 0; j < n_; j++) (*this)(i, j) = row(0, j);
}

void matrix::set_col(uint j, const matrix& col) {
    if (col.n_ != 1 || col.m_ != m_)
        throw bad_size();
    for (uint i = 0; i < m_; i++) (*this)(i, j) = col(i, 0);
}
...
double matrix::trace() const {
    if (!is_square())
        throw not_square();
    double trace = 0;
    for (int i = 0; i < m_; i++) trace += (*this)(i, i);
    return trace;
}
...
double matrix::det() const {
    ...
    do {
        prod = 1;
        for (uint i = 0; i < n_; i++) prod *= (*this)(i, p[i]);
        prod *= sgn;
        sgn = -sgn;
        detsum += prod;
...
matrix matrix::minor_matrix() const {
    ...
    matrix mat(m_, m_);
    for (uint i = 0; i < m_; i++)
        for (uint j = 0; j < m_; j++) mat(i, j) = minordet(i, j);
    return mat;
}
...
double matrix::norm() const {
    double norm2;
    if (m_ == 1)
        norm2 = ((*this).transpose() * (*this)).scalar();
    else if (n_ == 1)
        norm2 = ((*this) * (*this).transpose()).scalar();
    else
        norm2 = ((*this).transpose() * (*this)).trace();
    return sqrt(norm2);
}
...
bool matrix::operator!=(const matrix& a) { return !operator==(a); }
...
matrix operator*(const matrix& a, const matrix& b) {
    ...
    for (uint i = 0; i < a.m(); i++)
        for (uint j = 0; j < b.n(); j++) {
            double sum = 0;
            for (uint k = 0; k < a.n(); k++) sum += a(i, k) * b(k, j);
            mat(i, j) = sum;
        }
    return mat;
}
